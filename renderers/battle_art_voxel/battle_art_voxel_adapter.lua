local BattleArtVoxelAdapter = {}

BattleArtVoxelAdapter.info = {
...
function BattleArtVoxelAdapter.detect(
    mod
)
...
function BattleArtVoxelAdapter.initialize(
    mod,
    loadLocalModule,
    Registry,
    CharacterRuntime
    ...
    ----------------------------------------------------------------
    -- Find Battle Art Voxel
    ----------------------------------------------------------------

    local ds =
        mod.find("BATTLE_ART_VOXEL_FORK")


    if not ds then

        mod.log:info(
            "Battle Art Voxel not installed"
        )
    ...
    if not (
        ...
        mod.log:error(
            "Battle Art Voxel API unavailable"
        )
    ...
    --
    -- This renderer alone converts those packages into Battle Art Voxel
    -- GPU resources.
    ----------------------------------------------------------------
    ...
    ----------------------------------------------------------------
    -- Battle Art Voxel GPU resource cache
    --
    ...
    ----------------------------------------------------------------
    -- Battle Art Voxel / Love2D texture realization
    ----------------------------------------------------------------
    ...
    ----------------------------------------------------------------
    -- Load and realize one asset for Battle Art Voxel
    ----------------------------------------------------------------
    ...
    -- Uses observable/public Battle Art Voxel module surfaces and 3D
    ...
    local function installCompat0199()
        ...
        --
        -- Fly is not a normal Battle Art Voxel character pose. Recomp
        -- draws a dedicated 2D bird FX while Battle Art Voxel removes
        ...
        if type(originalVoxelRender) ~= "function"
            ...
            mod.log:error(
                "Battle Art Voxel adapter: Recomp 0.1.99 compatibility surfaces unavailable"
            )
        ...
        local function facingToYaw(
            ...
            ------------------------------------------------------------
            -- Player + Battle Art Voxel free camera:
        ...
        -- Battle Art Voxel loaded. Wrap it separately so the FIRST shadow
        ...
        ----------------------------------------------------------------
        -- Battle Art identity association.
        --
        -- Battle Art does not draw character billboards in the same order
        -- as entity.pose() is evaluated. Associate each billboard with a
        -- captured pose by the sprite definition carried by the billboard
        -- itself. Per-definition cursors preserve duplicate NPCs and cycle
        -- safely if Battle Art performs another render/reflection pass.
        ----------------------------------------------------------------

        local function matchingPoseByDef(
            ctx,
            poses,
            def,
            cursorField
            ...
            local cursors = ctx[cursorField]
            local index = (cursors[def] or 0) + 1
        ...
        ----------------------------------------------------------------
        -- Battle Art visible identity association.
        --
        -- `sprite.def` alone is not enough: connected maps can contribute
        -- several NPCs using the same sprite definition, and Battle Art does
        -- not draw those cards in pose-capture order.
        --
        -- For the visible card, the model matrix itself carries the card's
        -- world position. In the normal third-person camera Battle Art builds:
        --
        --   X = px          (not mirrored)
        --   X = px + 16     (mirrored)
        --   Z = py + 8
        --
        -- Rank same-definition poses against that real transform. Frame and
        -- mirror equality are used as strong tie-breakers, but position is the
        -- stable identity. If the matrix is unavailable, fall back to the
        -- proven definition cursor.
        ----------------------------------------------------------------

        local function matchingVisiblePoseByModel(
            ctx,
            poses,
            def,
            billboardFrame,
            ...
            if type(model) ~= "table"
                or type(model[4]) ~= "number"
                or type(model[12]) ~= "number" then

                return matchingPoseByDef(
                    ctx,
                    poses,
                    def,
                    "visibleDefCursors"
            ...
            for _, p in ipairs(
                ...
                if p
                    ...
                    --------------------------------------------------------
                    -- The visible mesh already identifies the exact frame.
                    -- Do not let a distant same-sprite NPC with a different
                    -- frame win merely because animations changed this tick.
                    --------------------------------------------------------

                    if poseFrame ~= billboardFrame then
                        score =
                            score + 4096
                    end
        ...
        Voxel3D.draw =
            function(
                ...
                --------------------------------------------------------
                -- Fly visible geometry is NOT injected here.
                --
                -- Battle Art can reach this hook in several semantic passes.
                -- The previous adapter marked Fly as drawn here even when the
                -- resulting geometry was not visible. The final scene boundary
                -- below is a guaranteed, unambiguous visible-scene seam.
                --------------------------------------------------------
        ...
        ----------------------------------------------------------------
        -- Fly visible injection at the guaranteed scene boundary.
        --
        -- Battle Art performs every visible world draw between beginScene()
        -- and endScene(). Injecting immediately before the original endScene
        -- avoids depending on the presence/order of ordinary character cards.
        -- This is the same lifecycle principle that proved reliable on the
        -- other renderer integrations.
        ----------------------------------------------------------------

        if type(originalVoxelEndScene) == "function" then
        ...
        VoxelScene.render =
            function(
                ...
                --------------------------------------------------------
                -- Ghost entities first, matching Battle Art Voxel posesOf().
                --------------------------------------------------------
                ...
                -- exposed by Battle Art Voxel as the map's authored FIGURE.
                --
                ...
                --------------------------------------------------------
                -- Let Battle Art Voxel execute its normal renderer.
                --------------------------------------------------------
    ...
    local function installCompat0183()
        ...
        if type(drawCast)
            ...
            mod.log:error(
                "Battle Art Voxel adapter: drawCast was not found"
            )
        ...
        if not viewFacingIndex
            ...
            mod.log:error(
                "Battle Art Voxel adapter: viewFacing was not found"
            )
        ...
        if not viewFacingResult then
            ...
            mod.log:error(
                "Battle Art Voxel adapter could not track current pose"
            )
        ...
        if not entityIndex
            ...
            mod.log:error(
                "Battle Art Voxel adapter: drawEntity was not found"
            )
        ...
        local function drawEntity(
            ...
            ------------------------------------------------------------
            -- `facing` may be Battle Art Voxel viewFacing.
            --
            ...
            ----------------------------------------------------------------
            -- No 3D asset:
            -- preserve Battle Art Voxel's original 2D behaviour.
            ----------------------------------------------------------------
        ...
        if not result then
            ...
            mod.log:error(
                "Battle Art Voxel adapter could not replace drawEntity"
            )
    ...
    if installCompat0199() then
        ...
        mod.log:info(
            "Battle Art Voxel Porygonal adapter installed"
        )
    ...
    mod.log:error(
        "Battle Art Voxel adapter: no compatible Recomp integration found"
    )
